...
test('shouldIndex accepts real project source', () => {
  ...
  assert.equal(cg.shouldIndex('C:/GIT/vaultflow/.claude/helpers/db.cjs'), true);
...
test('purgeCodeGraph removes rows for files that no longer exist on disk', () => {
  const root = freshDb();
  const conn = db.raw();
  const exists  = fileURLToPath(import.meta.url);              // this test file — real, indexable
...
test('purgeCodeGraph with no junk and existence off is a no-op', () => {
  freshDb();
  const conn = db.raw();
  const real = 'C:/GIT/vaultflow/.claude/helpers/db.cjs';
  seedFile(conn, real);
  const res = cg.purgeCodeGraph(db, { checkExistence: false });
