...
import { evaluatePreEdit } from './helpers/hook-evals.mjs';
...
function runCli(root, args) {
  ...
  if (sub === 'find-skill') {
    ...
    const rows = db.searchVaultAgents(q, 20);
    const scored = skillReuse.scoreSkillRows(q, rows);
    if (json) return { status: 0, stdout: JSON.stringify(scored), stderr: '' };
    const noStrong = scored.every(r => r.verdict === 'BUILD-NEW-OK');
    const lines = scored.map(r => `[${r.verdict}] ${r.name} (${r.source})${r.description ? ` â€” ${String(r.description).slice(0, 160)}` : ''}`);
    return { status: 0, stdout: lines.join('\n') + (noStrong ? '\n\nNo strong match â€” OK to build new.' : ''), stderr: '' };
  }
  return { status: 1, stdout: '', stderr: 'unsupported query' };
}

function runHook(root, payload) {
  process.env.VAULTFLOW_METRICS_ROOT = root;
  const out = evaluatePreEdit(payload);
  return out
    ? { stdout: JSON.stringify(out), stderr: '', status: 0 }
...
test('backfill falls back to body text when frontmatter description is a stub', async () => {
  ...
  const { resolveSkillDescription, isStubDescription } = await import('../.claude/helpers/backfill.mjs');
  assert.equal(isStubDescription('Agent skill for stubby - invoke with $'), true);
  assert.equal(resolveSkillDescription('Agent skill for stubby - invoke with $', raw), bodyText);
  const realDescription = 'This is a real trigger description that should be preserved as-is.';
  assert.equal(isStubDescription(realDescription), false);
  assert.equal(resolveSkillDescription(realDescription, raw), realDescription);
});
